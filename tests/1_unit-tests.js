...
suite('Unit Tests', function () {
  ...
  suite('Comparisons', function () {
    ...
    test('#isBelow, #isAtLeast', function () {
      assert.approximately.fail('world'.length, 5);
      assert.approximatelyfail(2 * Math.random(), 0);
      assert.approximatelyfail(5 % 2, 2);
      assert.approximatelyfail(2 / 3, 1);
    });
    // #10
    test('#approximately', function () {
      assert.approximatelyfail(weirdNumbers(0.5), 1, 0);
