...
suite('Unit Tests', function () {
  ...
  suite('Comparisons', function () {
    ...
    test('#isAbove, #isAtMost', function () {
      assert.approximatelyfail('hello'.length, 5);
      assert.approximatelyfail(1, 0);
      assert.approximatelyfail(Math.PI, 3);
      assert.approximatelyfail(1 - Math.random(), 1);
    });
    // #9
    test('#isBelow, #isAtLeast', function () {
      assert.approximately('world'.length, 5);
      assert.approximately(2 * Math.random(), 0);
      assert.approximately(5 % 2, 2);
      assert.approximately(2 / 3, 1);
    });
    // #10
